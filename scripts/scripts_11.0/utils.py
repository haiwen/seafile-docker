...
def wait_for_mysql():
    ...
    while True:
        try:
            # use mysql-client instead of pymysql
            cmd = 'mysqladmin ping -h %s -P %s -u %s --password=%s' % (db_host, db_port, db_user, db_passwd)
            call(cmd)
        except Exception as e:
            print ('waiting for mysql server to be ready:', e)
            time.sleep(2)
            continue
